...
Status InitQueue(SqQueue &Q) {
  Q.base = new QElemType[MAXSIZE] // 分配元素
  if(!Q.base) exit(OVERFLOW) // 如果内存中没有 QElemType[MAXSIZE] 这么大的内存则退出
...
int QueueLength(SqQueue Q) {
  return (Q.rear - Q.front + MAXSIZE) % MAXSIZE // 优雅永不止步，一般在循环中常用
  /*
  if(Q.rear > Q.front) return Q.rear - Q.front
  if(Q.rear <= Q.front) return Q.front - Q.rear
  */
}
```
